...
> Normalmente, cuando hacemos análisis de algoritmos, la complejidad asíntótica se da para el **peor de los casos**, es decir, el rendimiento que tendría nuestros algoritmos si todas las condiciones que lo harían más eficiente **no** son favorables. 
> 
> De forma intuitiva se ve a que no aporta mucha información decir cómo de eficiente es un algoritmo cuando las condiciones son totalmente favorables. Por ejemplo, decir que nuestros algoritmo tiene complejidad constante $\Omicron(1)$ a la hora de ordenar un conjunto de datos porque la lista ya está previamente ordenada no aporta ningún valor puesto que no podemos sacar información a partir de eso.
